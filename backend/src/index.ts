...
const createAuditLog = (action: string, metadata?: any, uid?: string) => {
    db`INSERT INTO audit_logs (action, metadata, user_id) VALUES (${action}, ${metadata ? JSON.stringify(metadata) : null}, ${uid ? uid : null})`
        .catch((e) => {
...
app.post("/register", async (req, res) => {
    ...
    let uv = null;
    try {
        uv = await db`INSERT INTO users (email, password_hash) VALUES (${sanitizeInput(email)}, ${passwordHash})`;
    } catch (e) {
        console.error(e)
...
app.post('/login', async (req, res) => {
    ...
    const { email, password } = parsed.data
    const user = await db`SELECT * FROM users WHERE email = ${sanitizeInput(email)}`
    if (user.length === 0) {
...
app.get('/my-db-info', async (req, res) => {
    ...
    const user = await db`SELECT id, email, created_at FROM users WHERE email = ${sanitizeInput(email)}`;
    if (user.length === 0) {
